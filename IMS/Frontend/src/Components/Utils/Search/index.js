...
export default function Index({search,handleResult}){ 
  ...
  useEffect(()=>{
    ...
    const getMaintenanceproducts = async() => {
   
      const  res =await axios.get(`maintenance/getmaintenanceitems`);
      // console.log(res);
      setMaintenanceproducts(res.data.maintenanceitems);
    ...
    getreselledProducts();
  },[Maintenanceproducts,Agencies,Vendors,Locations,Categories,result,Removedproducts,Products,Reselledproducts])
  let data=[];
  ...
  if(search === "Maintenance"){
    let i=0;
    let n= Maintenanceproducts.length;
    
    for(i=0;i<n;i++)
    {
      data.push(Maintenanceproducts[i].name);
    }
